...
The Saved Projects capability allows users to capture, store, and restore collections of browser tabs as projects in Raindrop. Users can save highlighted tabs with their context (pinned state and order), manage projects from the popup, and restore project workspaces with a single click.
## Requirements
...
- **AND** every item MUST include JSON metadata (`excerpt`) capturing the tab’s pinned state and tab index so it can be restored later.

#### Scenario: Provide result feedback
- **GIVEN** the save completes,
- **THEN** the background MUST return a `SaveProjectResult` with created/skipped/failed counts and the resolved project name, update the action badge, and the popup MUST surface the success/failure summary and refresh the saved-projects list.
...
- **AND** the background MUST append new Raindrop items (skipping URLs already present), include proper metadata, update counts, and return the summary so the popup can show the status.
...
- **AND** the background MUST delete the Raindrop collection and invalidate the cache so the popup can confirm success (or restore the row on error).

### Requirement: Users MUST be able to open or restore saved projects from the popup
Users MUST be able to either open the Raindrop page for a project or restore the stored tabs (respecting pinned order) into their browser window.
...
#### Scenario: Restore project tabs into the current window
...
- **AND** it MUST pin tabs flagged as pinned and report counts of created/pinned tabs alongside any errors so the popup can show status to the user.
...
- **THEN** the background MUST return `ok: false` with an error message, while the popup re-enables controls and shows the error in `statusMessage`.

### Requirement: Raindrop search MUST include saved projects
The popup's search functionality MUST include saved projects in search results, allowing users to discover and access projects alongside Raindrop items.

#### Scenario: Search projects by title
- **GIVEN** the user types a search query in the popup search input (at least 3 characters),
- **THEN** the system MUST query both Raindrop items and saved projects,
...
#### Scenario: Sort projects before Raindrop items in results
- **GIVEN** the search returns both matching projects and matching Raindrop items,
- **THEN** the results MUST be sorted with all matching saved projects appearing before any matching Raindrop items,
- **AND** within projects, maintain relevance-based ordering,
- **AND** within Raindrop items, maintain the existing title-match priority ordering.
...
- **AND** project results MUST be visually distinguishable from Raindrop item results.
...
- **GIVEN** search results include both projects and Raindrop items,
...
- **AND** pressing Enter on a highlighted item MUST trigger the appropriate action (restore project or open Raindrop item).

#### Scenario: Handle empty project results gracefully
- **GIVEN** the search query matches Raindrop items but no projects,
- **THEN** the system MUST display only Raindrop item results without errors,
...
- **GIVEN** the user views the popup search input,
- **THEN** the placeholder text MUST indicate that Raindrop and Notion content is searchable.
